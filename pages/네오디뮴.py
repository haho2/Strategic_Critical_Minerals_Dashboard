...
# 광물 설정
# =========================
PRICE_MINERAL_NAME    = "네오디뮴"
IMPORT_MINERAL_NAME   = "희토류"
NEWS_MINERAL_KEYWORDS = ("네오디뮴", "희토류")
...
try:
    price_info,  price_df,  price_table_df  = cached_load_price_data(PRICE_MINERAL_NAME)
    import_info, import_df, country_df, import_table_df = cached_load_import_data(IMPORT_MINERAL_NAME)
except Exception as e:
    st.error(f"DB 조회 오류: {e}"); st.stop()

if price_info  is None: st.error(f"'{PRICE_MINERAL_NAME}' 가격 데이터를 찾지 못했습니다.");  st.stop()
if import_info is None: st.error(f"'{IMPORT_MINERAL_NAME}' 수입량 데이터를 찾지 못했습니다."); st.stop()
